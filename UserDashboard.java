...
//     // BOOK SLOT
// // BOOK SLOT (updated to return bookingId and call PaymentGateway)
...
//         // Create initial booking entry with pending payment (no start_time)
//         PreparedStatement pst = con.prepareStatement(
...
//         // Pass bookingId to PaymentGateway
...
public class UserDashboard extends JFrame {
    ...
    // --------------------------------------
    // LOAD SLOTS BY SELECTED LOCATION
    // --------------------------------------
    ...
    // --------------------------------------
    // BOOK SLOT (NO CHANGE TO YOUR LOGIC)
    // --------------------------------------
    private void bookSelectedSlot() {
        int row = slotTable.getSelectedRow();
        if (row == -1) {
